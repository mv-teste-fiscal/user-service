...
class UserServiceTest {
    ...
    void deleteUser_HasTasks_ThrowsException() {
        when(userRepository.findById(1L)).thenReturn(Optional.of(user));
        when(taskClient.getTasksByUserId(1L)).thenReturn(List.of(new TaskDTO("teste", "Teste tarefa", "Finalizada")));

        Exception exception = assertThrows(RuntimeException.class, () -> userService.deleteUser(1L));

        assertTrue(exception.getMessage().contains("Usuário não pode ser deleteado"));
